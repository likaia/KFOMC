...
public class Test
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		/*		 获取元素重复出现次数,并计算每个元素的总占比,计算当前元素的总金额开始
		HashMap<String, Integer> length = test.getJSONArrayNum(d); // 传源数据进来
		JSONArray keyArr = new JSONArray(); // 键数组
		JSONArray itemArr = new JSONArray(); // 值数组
		// 获取hashMap中的key名
		...
		if (totalVal > 0)
		{
			...
			for (int i = 0; i < keyArr.length(); i++)
			{
				...
				for(int j = 0;j < totalAmount.length();j++)
				{
					JSONObject  totalAmountObj = totalAmount.getJSONObject(i);
					String totalName = totalAmountObj.getString("name");
					if(totalName.equals(keyName))
					{
						Double itemAmount = totalAmountObj.getDouble("amount");
		 ...
		 获取元素重复出现次数,并计算每个元素的总占比,计算当前元素的总金额结束*/
		
		/* 获取hashMap中的key名结束 */
		/*
		 * LkCommon test = new LkCommon(); String b =
		 * "[{\"name\":\"支付宝\",\"amount\":500.54},{\"name\":\"现金\",\"amount\":700.13},{\"name\":\"现金\",\"amount\":200.00},{\"name\":\"支付宝\",\"amount\":800.00},{\"name\":\"支付宝\",\"amount\":900.00}]";
		 * JSONArray d = new JSONArray(b); // --->把JSONString传换为JSONArray
		 * 
		 * test.getJSONArrayMoney(d); JSONArray归类 JSONArray aa=
		 * test.getJSONArray(d); //传源数据进来 DecimalFormat df=new
		 * DecimalFormat("0.00");//--->设置保留位数
		 * //System.out.println("归类好的数据:"+aa.toString()); JSONArray
		 * statisticsArr = new JSONArray(); //--->存放当前归类好的数据,每一类的支付方式,合计金额,重复次数
		 * int statisNum = 0; //--->重复次数 double totleAmount = 0.00;
		 * //---->当前金额总计 for(int i = 0;i<aa.length();i++) { JSONArray bb =
		 * aa.getJSONArray(i); //--->第一层数组的数据 if(bb.length()>1) { JSONObject
		 * statisticsObj = new JSONObject(); String name = ""; //取出当前重复数据的 金额
		 * 支付方式 for(int j = 0;j<bb.length();j++) { statisNum++; JSONObject bbObj
		 * = bb.getJSONObject(j); name = bbObj.getString("name"); double amount
		 * = bbObj.getDouble("amount"); df.format(amount); totleAmount +=
		 * amount; }
		 * System.out.println("当前重复的支付方式:"+name+",当前金额："+df.format(totleAmount)+
		 * ",当前支付方式重复次数:"+statisNum); statisticsObj.put("name", name);
		 * //--->当前重复数据的支付方式 statisticsObj.put("payRepeat",
		 * totleAmount);//--->当前重复数据的总金额 statisticsObj.put("statisNum",
		 * statisNum);///--->当前支付方式重复次数
		 * statisticsArr.put(statisticsObj);//--->将计算好的对象put进最终数组数据 statisNum =
		 * 0; //--->重复次数归零 totleAmount = 0;//--->当前总金额归零 } else { JSONObject
		 * notRepeatedObj = new JSONObject(); String name = ""; //取出当前未重复数据的 金额
		 * 支付方式 for(int j = 0;j<bb.length();j++) { statisNum++; JSONObject bbObj
		 * = bb.getJSONObject(j); name = bbObj.getString("name"); double amount
		 * = bbObj.getDouble("amount"); df.format(amount); totleAmount +=
		 * amount; }
		 * System.out.println("当前未重复的支付方式:"+name+",当前金额："+df.format(totleAmount)
		 * +",当前支付方式重复次数:"+statisNum); notRepeatedObj.put("name", name);
		 * //--->当前重复数据的支付方式 notRepeatedObj.put("payRepeat",
		 * totleAmount);//--->当前重复数据的总金额 notRepeatedObj.put("statisNum",
		 * statisNum);///--->当前支付方式重复次数 statisticsArr.put(notRepeatedObj);
		 * //--->将计算好的对象put进最终数组数据 statisNum = 0; //--->重复次数归零 totleAmount =
		 * 0;//--->当前总金额归零 } }
		 ...
		 * 
		 * 
		 * //获取元素重复出现次数 HashMap<String, Integer> length =
		 * test.getJSONArrayNum(d); //传源数据进来 JSONArray keyArr = new JSONArray();
		 * //键数组 JSONArray itemArr = new JSONArray(); //值数组 获取hashMap中的key名
		 * Set<String> mapSet = length.keySet();//获取所有的key值 为set的集合
		 * Iterator<String> itor = mapSet.iterator();// 获取key的Iterator遍历 int
		 * totalVal = 0; while (itor.hasNext()) {// 存在下一个值 JSONObject keyObj =
		 * new JSONObject(); //键对象 JSONObject itemObj = new JSONObject(); //值对象
		 * String key = itor.next();// --->当前key值 int val = length.get(key);
		 * //--->当前key对应的键值 keyObj.put("name", key); itemObj.put("name", val);
		 * totalVal+=val;//--->计算总数 keyArr.put(keyObj);//--->put键对象
		 * itemArr.put(itemObj);//--->put值对象 }
		 * System.out.println("当前元素总占比"+totalVal); if(totalVal>0) {
		 * 取出键数组的数值,取出值数组中的数值 for(int i = 0; i <keyArr.length();i++) {
		 * JSONObject keyObj = keyArr.getJSONObject(i); JSONObject itemObj =
		 * itemArr.getJSONObject(i); int itemVal = itemObj.getInt("name");
		 * //计算当前元素所占百分比 String percentage = test.txPercentage(itemVal,
		 * totalVal); double percentageDouble =
		 * Double.valueOf(percentage.toString()); int percentageInt =
		 * (int)Math.floor(percentageDouble*100); //当前每个元素占比
